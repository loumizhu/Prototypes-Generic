...
(function(){
  ...
  function buildSelect(sel){
    ...
    function positionMenu(){
      ...
      menu.style.left=tr.left+'px'; menu.style.width=tr.width+'px';
...
/* ═══ Dark/Light theme switch + light-theme palettes ═══════════════════════════
   Ported from the Super Admin. Sun = switch to light, Moon = switch to dark;
   navigates between a page and its -Light twin. Loaded by every page, so the
   switch is global. A header toggle auto-mounts (into .pg-actions / header bar /
   breadcrumb, else fixed top-right); Settings shows in-page controls instead. */
...
window.GS_LIGHT_PRESET_KEY = 'gs-ra-light-preset';
(function(){
