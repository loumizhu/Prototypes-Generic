/* ═══════════════════════════════════════════════════════════════════════════
   GreenStreets Retailer Admin — light-preview glue  (greenstreets-light.js)
   ═══════════════════════════════════════════════════════════════════════════
   Loaded ONLY by the *-Light.html pages, after retailer-admin.js + the theme JS.
   ...
   Mirror of the Super Admin greenstreets-light.js (RA filenames + RA logo).
   ═════════════════════════════════════════════════════════════════════════ */
(function(){
  var LOGO='img/GreenStreet-Light-Theme.png';
  var FILE_RX=/Greenstreets_retailer_admin_[A-Za-z0-9-]+?\.html/;

  /* darkfile.html → darkfile-Light.html (leaves query/hash intact, skips already-light) */
  function toLight(href){
    return href.replace(/(Greenstreets_retailer_admin_[A-Za-z0-9-]+?)(\.html)/g,function(m,p1,p2){
  ...
  function rewriteInlineNav(root){
    (root||document).querySelectorAll('[onclick]').forEach(function(el){
      ...
      if(!oc || oc.indexOf('Greenstreets_retailer_admin_')<0) return;
      var nu=toLight(oc);
