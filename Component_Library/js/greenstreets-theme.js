...
/* ═══ Dark/Light theme switch + light-theme palettes ═══════════════════════════
   Ported from the Retailer Admin / Retailer User portals. Sun = switch to
   light, Moon = switch to dark; navigates between a page and its -Light
   twin. Loaded by every page, so the switch is global. A header toggle
   auto-mounts into the .ph header bar's right-hand action group; Settings
...
window.GS_LIGHT_PRESET_KEY = 'gs-sp-light-preset';
(function(){
  ...
  function mountToggle(){
    ...
    /* size the toggle to exactly match its neighbours' height, so it never
       sticks out taller than the row of buttons it sits amongst */
    ...
    /* Supplier Portal's page header is .ph with two children: the company-name
       block and a right-hand action group (Getting started / Settings gear).
       Drop the toggle into that action group so it sits alongside them. */
    var bar = q('.ph');
    if(bar){
      ...
      if(kids.length >= 2){
        ...
        grp.style.gap = '10px';
        btn.classList.add('hdr-theme-inrow'); grp.appendChild(btn); fitHeight(grp); return;
      }
