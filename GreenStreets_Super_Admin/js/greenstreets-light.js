/* ═══════════════════════════════════════════════════════════════════════════
   GreenStreets Super Admin — light-preview glue  (greenstreets-light.js)
   ═══════════════════════════════════════════════════════════════════════════
   Loaded ONLY by the *-Light.html pages, after super-admin.js. It makes the whole
   light set work together as one preview:
     ...
     3. rewrites in-page navigation (anchor href + inline onclick="location.href=…")
        to the light twin, so packaging tiles / detail links stay in the light set,
     4. injects a "back to dark" banner on pages that don't already ship one.
   Links/handlers flagged data-nolight (the "back to dark" links) are left alone.
   There is nothing per-page to wire — a light page just needs body.lt + the light
   CSS + this script. ═══════════════════════════════════════════════════════════ */
(function(){
  var LOGO='../img/GreenStreet-Light-Theme.png';
  var FILE_RX=/greenstreets_super_admin_[A-Za-z0-9-]+?\.html/;

  /* darkfile.html → darkfile-Light.html (leaves query/hash intact, skips already-light) */
  function toLight(href){
    return href.replace(/(greenstreets_super_admin_[A-Za-z0-9-]+?)(\.html)/g,function(m,p1,p2){
  ...
  function swapLogos(){
    document.querySelectorAll('.sb-logo,.gs-logo-img,img[alt="Greenstreets"]').forEach(function(im){
  ...
  /* 3b — rewrite inline onclick="location.href='…'" navigation (packaging tiles etc.) */
  function rewriteInlineNav(root){
    (root||document).querySelectorAll('[onclick]').forEach(function(el){
      var oc=el.getAttribute('onclick');
      if(!oc || oc.indexOf('greenstreets_super_admin_')<0) return;
      var nu=toLight(oc);
